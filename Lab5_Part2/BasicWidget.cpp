...
void BasicWidget::initializeGL()
{
  ...
  ibo_.create();
  ibo_.bind();
  ibo_.allocate(idx, numOfVertx * sizeof(GL_FLOAT));
  // ENDTODO
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, 5 * sizeof(GL_FLOAT), idx, GL_STATIC_DRAW);
  // ENDTODO
...
void BasicWidget::paintGL()
{
  ...
  glDrawElements(GL_TRIANGLES, numOfVertx, GL_UNSIGNED_INT, 0);
  // ENDTODO
  vao_.release();
  shaderProgram_.release();
#else
